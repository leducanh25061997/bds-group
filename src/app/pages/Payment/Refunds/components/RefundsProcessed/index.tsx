...
export const RefundProcessed = () => {
  ...
  const { refundsApprove, isLoading, paramsSearch } = useSelector(selectRefund);
  ...
  const fetchDataForPage = useCallback(
    (params: FilterParams) => {
      dispatch(actions.fetchRefundApprove(params));
    },
    [actions, dispatch],
  );
  ...
  const renderItem = (item: RefundsResponse, index: number) => {
    return [
      ...
      <EllipsisText
        text={`${
          item.status === 'APPROVED' ? item.note || '' : item.reason || ''
        }`}
        line={1}
      />,
  ...
  return (
    ...
    >
      <Table
        ...
        items={refundsApprove ? refundsApprove?.data || [] : undefined}
        pageNumber={filterParams.page}
        totalElements={refundsApprove?.total}
